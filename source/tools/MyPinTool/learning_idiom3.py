    ...
    if instructions[index_1].accessed_offset == instructions[index_2].accessed_offset:
        if instructions[index_1].thread_id != instructions[index_2].thread_id:
            if instructions[index_1].is_write == "1" and instructions[index_2].is_read == "1":
                reward = 1.5
            elif instructions[index_1].is_read == "1" and instructions[index_2].is_write == "1":
                reward = -1
            elif instructions[index_1].is_read == "1" and instructions[index_2].is_read == "1":
                reward = -1
        elif instructions[index_1].thread_id == instructions[index_2].thread_id:
            if instructions[index_1].is_read == "1" and instructions[index_2].is_write == "1":
                reward = 1
            elif instructions[index_1].is_write == "1" and instructions[index_2].is_read == "1":
                reward = -1
...
try:
    with open("states_binary_idiom3", "rb") as pickle_file:
...
def update_q_value(current, next):
    ...
    if is_contained_current == 0 and is_contained_next == 0:
        ...
        states[-1].q_value[str(next)] = round(learning_rate * reward, 2)

    elif is_contained_current == 0 and is_contained_next == 1:
        create_state(current)
        states[-1].q_value[str(next)] = round(learning_rate * (reward + discount_factor * max(states[index_next].q_value.values())), 2)

    elif is_contained_current == 1 and is_contained_next == 1:
        if str(next) in states[index_current].q_value:
            states[index_current].q_value[str(next)] = round(states[index_current].q_value[str(next)] + learning_rate * (reward + discount_factor * max(states[index_next].q_value.values()) - states[index_current].q_value[str(next)]), 2)
        else:
            states[index_current].q_value[str(next)] = round(learning_rate * (reward + discount_factor * max(states[index_next].q_value.values())), 2)
...
with open("states_binary_idiom3", "wb") as pickle_file:
