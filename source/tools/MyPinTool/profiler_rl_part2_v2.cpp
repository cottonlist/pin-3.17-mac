...
VOID instrument(INS ins, void *v) {
	...
	if(threadId != 0){
		if ((isMemoryRead == 1 || isMemoryWrite==1) && isStackRead ==0 && isStackWrite == 0&&isControlFlow==0)
		{
			...
			instr[index].disassemble = disassemble;
			index++;
		}
	} else {
...
VOID Fini(INT32 code, VOID *v){
	...
	for (int i = 0; i < index; i++) {
		WriteFile1 << instr[i].memoryOffset << "; " << instr[i].isMemoryRead << "; " << instr[i].isMemoryWrite << "; " << instr[i].isStackRead << "; " << instr[i].isStackWrite << "; " << instr[i].disassemble << endl;
	}
}
