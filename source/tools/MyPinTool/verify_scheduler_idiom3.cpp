...
int interleaving[] = {15932, 16028, 16065, 15962, 15962, 16077, 16077, 15991};
...
VOID signalWrapperSemaphore4(THREADID threadId) {
	PIN_SemaphoreSet(&pinSemaphore4);
}
...
VOID waitWrapperSemaphore4(THREADID threadId) {
	PIN_SemaphoreWait(&pinSemaphore4);
}
...
VOID instrument(INS ins, void *v) {
	...
	if (offset == interleaving[0]) {
		...
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)waitWrapperSemaphore2, IARG_THREAD_ID, IARG_END);
	} else if (offset == interleaving[4]) {
		if (INS_IsValidForIpointAfter(ins)) {
			INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR)signalWrapperSemaphore3, IARG_THREAD_ID, IARG_END);
		}
	} else if (offset == interleaving[5]) {
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)waitWrapperSemaphore3, IARG_THREAD_ID, IARG_END);
	} else if (offset == interleaving[6]) {
		if (INS_IsValidForIpointAfter(ins)) {
			INS_InsertCall(ins, IPOINT_AFTER, (AFUNPTR)signalWrapperSemaphore4, IARG_THREAD_ID, IARG_END);
		}
	} else if (offset == interleaving[7]) {
		INS_InsertCall(ins, IPOINT_BEFORE, (AFUNPTR)waitWrapperSemaphore4, IARG_THREAD_ID, IARG_END);
	}
}
...
int main(int argc, char *argv[])
{
	...
	PIN_SemaphoreInit(&pinSemaphore4);
