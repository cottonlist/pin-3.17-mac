...
ifstream InFile1("instruction_count.log");
ifstream InFile2("numOfInsTh1");
ifstream InFile3("numOfInsTh2");
ofstream OutFile1("execution_order1");
...
int main(int argc, char const *argv[])
{
    ...
    while((fscanf(fp1, "%lld, %d", &totalIns[index].memoryOffset, &totalIns[index].threadId)) != EOF) {
        index++;
    }
    ...
    executionOrder1 = rand() % (lenOfTh1-1);
    executionOrder2 = rand() % (lenOfTh1-1);
    while (executionOrder2 == executionOrder1)
    {
        executionOrder2 = rand() % (lenOfTh1-1);
    }

    executionOrder3 = rand() % (lenOfTh2-1);
    executionOrder4 = rand() % (lenOfTh2-1);
    ...
    unsigned long long order1;
    unsigned long long order2;
    unsigned long long order3;
    unsigned long long order4;
